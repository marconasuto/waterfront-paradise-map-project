...
def test_download_file_verifies_expected_sha(tmp_path: Path, respx_mock: respx.Router) -> None:
    payload = b"x" * 1024
...
def test_download_file_raises_on_sha_mismatch(tmp_path: Path, respx_mock: respx.Router) -> None:
    payload = b"abc"
...
def test_download_file_raises_on_http_error(tmp_path: Path, respx_mock: respx.Router) -> None:
...
def test_download_file_passes_custom_headers(tmp_path: Path, respx_mock: respx.Router) -> None:
