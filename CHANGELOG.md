...
- 2026-05-27 — Phase 9: GitHub Pages deploy wiring.
  - `.github/workflows/deploy.yml`: on push to main affecting
    ...
    data/processed/style.json` (plus manual dispatch), install pnpm 10
    + Node 22, typecheck + test + build the webapp with the public
    token injected from `secrets.MAPBOX_PUBLIC_TOKEN`, deploy
    `webapp/dist/` via `actions/deploy-pages`.
  - `webapp/vite.config.ts` honours `VITE_BASE_PATH` so the GH Pages
    subpath works.
  - `webapp/src/paths.ts` exposes `assetUrl(rel)` composing relative
    URLs against `import.meta.env.BASE_URL`. All fetches in `main.ts`
    + `content/loader.ts` route through it.
  - `data/processed/style.json` now tracked in git so the CI build
    does not need the Python pipeline; deterministic + ~6 KB, so
    diffs stay small. Regenerated by `pixi run publish-style`.
  - `plans/09_deployment.md` rewritten with concrete step-by-step:
    create + push the repo, add the secret, enable Pages, first deploy,
    URL-restrict the token.
  - Suite still at 121 passed. Subpath build verified:
    `VITE_BASE_PATH=/manfredonia-map/ pnpm build` emits
    `<script src="/manfredonia-map/assets/…">`.
