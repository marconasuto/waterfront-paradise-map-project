...
| Version         | 0.9                                  |
| Created         | 2026-05-23                           |
| Last updated    | 2026-05-25 (Phase 4: 13 vector normalizers wired; OPEN-SIN-1 closed with authoritative MASE/ISPRA shapefile) |
| Owner           | Marco Nasuto                         |
...
| 5 | SIN Manfredonia (ex-Enichem)| vector | **Regione Puglia / InnovaPuglia open data** — *Siti di Interesse Nazionale (SIN)*, <https://dati.puglia.it/ckan/dataset/siti-di-interesse-nazionale-sin>. Shapefile bundle in EPSG:32633 covering all 4 Puglia SINs (Bari, Brindisi, Manfredonia, Taranto); the normalizer filters to `SITO == "MANFREDONIA"` (5 polygons: public landfills + ex-Enichem + marine areas). Wired in `acquisition/regione_puglia.py` for reproducible re-fetch. | 2024 (SIN-5 perimeter mod. 02/12/2024); dataset last updated 2025-08-04 | CC-BY-4.0 (cite *InnovaPuglia - Servizio Territorio e Ambiente*) |
...
| OPEN-SIN-1    | Authoritative MASE SIN-5 Manfredonia perimeter is not exposed via a public, programmatic URL. The SIN page on `bonifichesiticontaminati.mite.gov.it/sin-5/` shows a map image but no shapefile/GeoJSON/WFS link; MOSAICO/ReNDiS expose WFS only behind the ISPRA metadata catalog UI; `sgi2.isprambiente.it/geoserver` does not host SIN. Options: (a) formal data request to MASE/ISPRA, (b) manual digitization from the SIN-5 decree maps, (c) keep OSM `landuse=industrial` ("Zona Industriale di Manfredonia-Monte Sant'Angelo") as the v1 proxy. | user | **closed 2026-05-25** — the *Regione Puglia* open-data CKAN catalog publishes the SIN perimeters for all 4 Puglia SINs at <https://dati.puglia.it/ckan/dataset/siti-di-interesse-nazionale-sin> (CC-BY-4.0, dataset last updated 2025-08-04). Wired as `acquisition.regione_puglia.RegionePugliaSpec(dataset_id="sin")` + `mfd-map acquire regione-puglia dataset sin`. |
